...
string compress(const string& raw) {
  int best_len = 0; string best_sub = ""; int max_saved = 0;
  for (int len = 2; len <= 15; ++len) {
    ...
    for (auto& kv : counts) {
      int saved = (kv.second - 1) * (len - 1) - 2;
      if (saved > max_saved) {
        max_saved = saved; best_sub = kv.first; best_len = len;
      }
    }
  }
  ...
  for (int i = 0; i < (int)raw.size(); ) {
    if (i <= (int)raw.size() - best_len && raw.substr(i, best_len) == best_sub) {
      if (first) { res += "M" + best_sub + "M"; first = false; }
      else { res += "P"; }
      i += best_len;
    }
    else { res += raw[i]; i++; }
