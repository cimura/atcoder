...
int main() {
  ...
  int ans = 0;
  for (int l = 1; l <= n; ++l) {
    auto it1 = lower_bound(A.begin() + l, A.end(), a + A[l - 1]);
    int r_min = distance(A.begin(), it1);

    auto it2 = lower_bound(B.begin() + l, B.end(), b + B[l - 1]);
    int r_limit = distance(B.begin(), it2);

    ans += max(0, r_limit - r_min);
  }
  cout << ans << endl;
}
