...
int main() {
  ...
  ll diff_r = 0;
  ll max_r = 0, min_r = 10e10;
  rep(i, n) {
    max_r = max(max_r, v[0][i]);
    min_r = min(min_r, v[0][i]);
  }
  diff_r = (max_r - min_r + 1) / 2;

  ll diff_c = 0;
  ll max_c = 0, min_c = 10e10;
  rep(i, n) {
    max_c = max(max_c, v[1][i]);
    min_c = min(min_c, v[1][i]);
  }

  diff_c = (max_c - min_c + 1) / 2;

  cout << max(diff_r, diff_c) << endl;
}
